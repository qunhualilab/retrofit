...
Theta       = vignetteColonData$a3_results_4k_iterations$decompose$th
################################################################################
...
```{r}
# fetal12_genes=as.character(read.csv("~/Downloads/fig6b/Fetal12-relevantgenes.csv",header = F)[,1])
# adult_genes=as.character(read.csv("~/Downloads/fig6b/Adult-relevantgenes.csv",header = F)[,1])
fetal12_genes = vignetteColonData$fetal12_genes
adult_genes   = vignetteColonData$adult_genes
a1_w          = vignetteColonData$a1_results_4k_iterations$annotateWithCorrelations$w
a3_w          = vignetteColonData$a3_results_4k_iterations$annotateWithCorrelations$w
# a3_w          = vignetteColonData$a1_results_4k_iterations$annotateWithCorrelations$w
# a1_w          = vignetteColonData$a3_results_4k_iterations$annotateWithCorrelations$w
genes         = sort(unique(c(fetal12_genes, adult_genes)), decreasing=FALSE)
df            = vignetteColonData$marker_ref_df
# genes         = genes[genes %in% row.names(W_mark_prop)]
```

Plot the normalized gene expression for these genes estimated by RETROFIT:
```{r}
# df=read.csv("Colon_markers.csv")[,-1]
df=vignetteColonData$marker_ref_df

# gene1=as.character(read.csv("Fetal12-relevantgenes.csv",header = F)[,1])
gene1=vignetteColonData$fetal12_genes
# gene3=as.character(read.csv("Adult-relevantgenes.csv",header = F)[,1])
gene3=vignetteColonData$adult_genes
genes=sort(unique(c(gene1,gene3)),decreasing = F)

# W1=read.csv("A3_W_processed.csv",row.names = 1)
# W3=read.csv("A1_W_processed.csv",row.names = 1)
