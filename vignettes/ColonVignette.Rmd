...
We assign components to the cell type it has maximum correlation with as shown in figure above. Finally, cell-type proportions for each spot in the ST data (X) are stored in H_mark_norm of order K x S.
...
for(i in 1:ncol(H_mark)){
    H_mark_norm[,i]=H_mark[,i]/sum(H_mark[,i])
}
W_mark_norm <- W_mark
for(i in 1:nrow(W_mark)){
    W_mark_norm[i,]=W_mark[i,]/sum(W_mark[i,])
}
```
...
We assign components to the cell type it has maximum average marker expression in, as shown in figure above. Finally, cell-type proportions for each spot in the ST data (X) are stored in H_mark_norm of order K x S.
...
```{r}
rowSums(H_mark_norm)/ncol(H_mark_norm)
```
...
```{r}
dat=as.data.frame(cbind(coords,t(H_mark_norm)))
colnames(dat)=c(colnames(coords),rownames(H_mark_norm))
...
df=as.data.frame(t(H_mark_norm[c(2,4,6,5,7,1,8,3),]))
...
```{r}
df=as.matrix(W_mark_norm[genes,])
