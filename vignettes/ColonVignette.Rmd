...
```{r, normalize annotated with marker}
# normalize
H_mark_norm <- H_mark
...
```{r}
fetal12_genes = vignetteColonData$fetal12_genes
adult_genes   = vignetteColonData$adult_genes
genes         = sort(unique(c(fetal12_genes, adult_genes)), decreasing = F)
genes         = genes[genes %in% row.names(W_mark_norm)]
```
